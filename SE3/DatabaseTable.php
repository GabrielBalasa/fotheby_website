<?php 
    ...
    class DatabaseTable
    {
        ...
        public function findOne($field, $value) //Find items in database table based on one field
        {
        ...
        // public function findJoins($tableOne, $tableTwo, $joinValueOne, $joinValueTwo, $field, $value) //Find items in database table based on one field and joins
        // {
        //     $stmt = $this->pdo->prepare(
        //     'SELECT * FROM ' . $this->table . ' JOIN ' . $tableOne . ' ON ' . $this->table . '.' . $joinValueOne . ' = ' . $tableOne . '.' . $joinValueOne . ' JOIN ' .$tableTwo . ' ON '  . $this->table . '.' . $joinValueTwo . ' = ' . $tableTwo . '.' . $joinValueTwo . ' WHERE ' . $field . ' = :value');
        ...
        public function orderCurrent($field, $value, $field2, $value2) // Order items in database table based on one field which is less than a value
        {
            $stmt = $this->pdo->prepare('SELECT * FROM ' . $this->table . ' WHERE ' . $field . ' < :value AND ' . $field2 . ' > :value2 ORDER BY ' . $field . ' DESC');
            $stmt->setFetchMode(\PDO::FETCH_CLASS, $this->entityClass, $this->entityConstructor);
            $values = [
                'value' => $value,
                'value2' => $value2
        ...
        public function orderCurrentAndUpcoming($field, $value) // Order items in database table based on one field which is less than a value
        {
            $stmt = $this->pdo->prepare('SELECT * FROM ' . $this->table . ' WHERE ' . $field . ' > :value ORDER BY ' . $field . ' DESC');
            $stmt->setFetchMode(\PDO::FETCH_CLASS, $this->entityClass, $this->entityConstructor);
            $values = [
                'value' => $value
            ];
        ...
        public function orderPast($field, $value) // Order items in database table based on one field which is less than a value
        {
            $stmt = $this->pdo->prepare('SELECT * FROM ' . $this->table . ' WHERE ' . $field . ' < :value ORDER BY ' . $field . ' DESC');
            $stmt->setFetchMode(\PDO::FETCH_CLASS, $this->entityClass, $this->entityConstructor);
            $values = [
        ...
        public function orderUpcoming($field, $value) // Order items in database table based on one field which is less than a value
        {
            $stmt = $this->pdo->prepare('SELECT * FROM ' . $this->table . ' WHERE ' . $field . ' > :value ORDER BY ' . $field . ' DESC');
            $stmt->setFetchMode(\PDO::FETCH_CLASS, $this->entityClass, $this->entityConstructor);
            $values = [
