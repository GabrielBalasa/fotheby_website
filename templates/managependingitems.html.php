...
<div class="col-10 px-5 pt-5 border-1">
    <h2 class="pb-4">Pending lot items</h2>
    ...
    <table id="example" class="table table-striped">
        ...
        <?php foreach ($items as $item) : ?>
            ...
            <tr>
                ...
                <td>
                    <form action="/item/accept" method="POST" >
                        <input hidden name="item[id]" value="<?=$item->id?>">
                        <input hidden name="item[status]" value="1">
                        <input type="submit" class="btn btn-outline-dark btn-sm" value="Accept">
                    </form>
                </td>
                <td><?=$item->id?></td>
                <td><?=$item->title?></td>
